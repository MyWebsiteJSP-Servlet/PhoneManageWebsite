...
/**
 * Servlet implementation class LoadProductByCategoriesController
 */
@WebServlet("/load-product-by-categories")
public class LoadProductByCategoriesController extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public LoadProductByCategoriesController() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (session != null) {
			...
			if (user != null) {
				...
				ArrayList<Product> lst_Res = proDao.selectProductByCategories(categories, pageInt);
				int tongSoSp = proDao.getTongSoTrangByCategories(categories);
				...
				request.setAttribute("nameRadio", categories);
				request.setAttribute("tongSoTrang", tongSoTrang);
				request.setAttribute("sourceServlet", "loadProductByCategories");
